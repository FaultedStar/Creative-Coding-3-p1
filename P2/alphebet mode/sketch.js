...
function drawFinger(x, y, tilt, colour, scale) {
 ...
 // pop();
  //ellipse(0,0,30,60)
	...
	function drawPalm(Ox, Oy,Pwidth, s){
	 var middle_x = canvasWidth / 3*2;
  var middle_y = canvasHeight / 2;
  resetMatrix();
  push();
...
function drawFromDataObject(x, y, s, obj) {
  ...
  translate(x, y);
  scale(s, s);
    drawPalm(obj['palm']['x'], obj['palm']['y'], obj['palm']['width'],s);
...
if (curDrawMode == "alphabet") {
    ...
    for(var j=0; j<4; j++) {
      for(var i=0; i<9; i++) {
        ...
        drawFromDataObject(hw + i * w2/9.0,hh + j * h2/4.0, 0.20, obj)
